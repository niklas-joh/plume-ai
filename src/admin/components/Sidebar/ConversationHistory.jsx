...
export default function ConversationHistory( {
	...
	if ( conversations.length === 0 ) {
		return (
			<div className="stilus-sidebar__empty">
				<p>No conversations yet.</p>
			</div>
		);
	}
	return (
		<nav className="stilus-conv-list">
			{ conversations.map( ( conv ) => (
				<div
					key={ conv.id }
					className={ `stilus-conv-item ${
						conv.id === activeId ? 'is-active' : ''
					}` }
				>
					<button
						className="stilus-conv-item__body"
						onClick={ () => onSelect( conv.id ) }
						type="button"
						title={ conv.title || __( 'Untitled', 'wp-ai-mind' ) }
					>
						<span className="stilus-conv-item__title">
							{ conv.title || 'Untitled' }
						</span>
						<span className="stilus-conv-item__date">
							{ conv.updated_at
								? new Date(
										conv.updated_at
					...
					{ deleteErrors[ conv.id ] && (
						<p className="stilus-conv-item__error">
							{ deleteErrors[ conv.id ] }
						</p>
					) }
					{ confirmingId === conv.id ? (
						<span className="stilus-conv-item__confirm">
							<button
								className="stilus-conv-item__confirm-yes stilus-btn stilus-btn--ghost"
								onClick={ () => {
							...
							<button
								className="stilus-conv-item__confirm-no stilus-btn stilus-btn--ghost"
						...
						<button
							className="stilus-conv-item__delete stilus-btn stilus-btn--ghost"
