...
test.describe('P2 — Dashboard landing page', () => {
    ...
    test('dashboard page renders with title and Start section', async ({ page }) => {
        ...
        await page.waitForSelector('.stilus-dash-title', { timeout: 10000 });
        await expect(page.locator('.stilus-dash-title')).toContainText('Stilus');
        await expect(page.locator('.stilus-dash-tiles')).toBeVisible();
        await expect(page.locator('.stilus-dash-resources')).toBeVisible();
        await expect(page.locator('.stilus-dash-footer')).toBeVisible();
    });
    ...
    test('usage widget is visible for administrator', async ({ page }) => {
        ...
        await expect(page.locator('.stilus-usage-widget')).toBeVisible();
    });
    ...
    test('Run setup again link is visible and wired to the PHP run_setup action', async ({ page }) => {
        ...
        const runSetupLink = page.locator('.stilus-dash-footer__link', { hasText: 'Run setup again' });
    ...
    test('all resource links have correct attributes', async ({ page }) => {
        ...
        const links = page.locator('.stilus-dash-resource');
