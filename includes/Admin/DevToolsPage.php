 ...
 *
 * Activated by defining PLUME_DEV_KEY in wp-config.php. The page never appears
...
class DevToolsPage {
	 ...
	 * @since 1.11.0
	 */
	private const OPTION_KEY_HASH = 'plume_dev_key_hash';

	/**
	 * Admin page slug used in the URL: wp-admin/admin.php?page=plume-dev-tools
	 *
	 * @since 1.11.0
	 */
	public const PAGE_SLUG = 'plume-dev-tools';
	...
	public static function add_page(): void {
		add_submenu_page(
			null,
			__( 'Developer Tools — Plume', 'plume' ),
			__( 'Dev Tools', 'plume' ),
			'manage_options',
			self::PAGE_SLUG,
			[ self::class, 'render' ]
	...
	/**
	 * Verify that PLUME_DEV_KEY is defined, non-empty, and matches the stored hash.
	 *
	...
	public static function is_active(): bool {
		if ( ! defined( 'PLUME_DEV_KEY' ) || '' === (string) PLUME_DEV_KEY ) {
			return false;
		}
		if ( ! current_user_can( 'manage_options' ) ) {
			return false;
		}
		$hash   = self::hash_key( (string) PLUME_DEV_KEY );
	...
	private static function enqueue_assets(): void {
		$asset_file = PLUME_DIR . 'assets/admin/dev-tools.asset.php';
		...
		wp_enqueue_script(
			'plume-dev-tools',
			PLUME_URL . 'assets/admin/dev-tools.js',
			$asset['dependencies'],
			$asset['version'],
			true
		);
	...
	public static function render(): void {
		if ( ! self::is_active() ) {
			wp_die(
				esc_html__( 'Developer tools are not enabled on this site.', 'plume' ),
				'',
				[ 'response' => 403 ]
			);
		}
		...
		if ( null === $usage['limit'] ) {
			$usage_display = __( 'Unlimited', 'plume' );
		} else {
			$usage_display = number_format_i18n( $usage['used'] ) . ' / ' . number_format_i18n( $usage['limit'] ) . ' ' . __( 'tokens', 'plume' );
		}
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Developer Tools', 'plume' ); ?></h1>

			<div class="notice notice-warning inline">
				<p>
					<strong><?php esc_html_e( 'Development use only.', 'plume' ); ?></strong>
					<?php esc_html_e( 'Changes here affect only local WordPress state. The Cloudflare proxy enforces real quotas independently.', 'plume' ); ?>
				</p>
			</div>

			<h2><?php esc_html_e( 'Current State', 'plume' ); ?></h2>
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><?php esc_html_e( 'Tier', 'plume' ); ?></th>
					<td id="plume-dev-tier-label">
						<strong><?php echo esc_html( $tier_labels[ $current_tier ] ?? $current_tier ); ?></strong>
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Usage this month', 'plume' ); ?></th>
					<td id="plume-dev-usage"><?php echo esc_html( $usage_display ); ?></td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Can use', 'plume' ); ?></th>
					<td id="plume-dev-can-use"><?php echo $usage['can_use'] ? '&#10003; ' . esc_html__( 'Yes', 'plume' ) : '&#10007; ' . esc_html__( 'No (limit reached)', 'plume' ); ?></td>
				</tr>
			</table>

			<h2><?php esc_html_e( 'Switch Tier', 'plume' ); ?></h2>
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row">
						<label for="plume-tier-select"><?php esc_html_e( 'Tier', 'plume' ); ?></label>
					</th>
					<td>
						<select id="plume-tier-select">
						...
						<button class="button button-primary" id="plume-apply-tier">
							<?php esc_html_e( 'Apply', 'plume' ); ?>
						</button>
					</td>
				</tr>
			</table>

			<h2><?php esc_html_e( 'Usage Controls', 'plume' ); ?></h2>
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><?php esc_html_e( 'Reset', 'plume' ); ?></th>
					<td>
						<button class="button" id="plume-reset-usage">
							<?php esc_html_e( 'Reset to zero', 'plume' ); ?>
						</button>
						<p class="description">
							<?php esc_html_e( "Clears this month's token counter — simulates a fresh month.", 'plume' ); ?>
						</p>
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Exhaust', 'plume' ); ?></th>
					<td>
						<button class="button" id="plume-set-ceiling">
							<?php esc_html_e( 'Set to ceiling', 'plume' ); ?>
						</button>
						<p class="description">
							<?php esc_html_e( "Sets usage to the current tier's monthly limit to trigger the blocked state.", 'plume' ); ?>
						</p>
					</td>
				</tr>
			</table>

			<div id="plume-dev-notice" style="display:none;" class="notice inline" aria-live="polite"></div>
		</div>
		<?php
	}
