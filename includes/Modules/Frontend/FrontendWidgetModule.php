...
namespace Plume\Modules\Frontend;
 ...
 *
 * The widget is mounted on any page that contains the [plume_chat] shortcode.
 */
class FrontendWidgetModule {
	...
	public static function register(): void {
		...
		\add_shortcode( 'plume_chat', [ self::class, 'render_shortcode' ] );
	}
	...
	public static function enqueue_assets(): void {
		$asset_file = PLUME_DIR . 'assets/frontend/widget.asset.php';
		...
		\wp_enqueue_script(
			'plume-widget',
			PLUME_URL . 'assets/frontend/widget.js',
		...
		\wp_enqueue_style(
			'plume-widget',
			PLUME_URL . 'assets/frontend/widget.css',
			[],
			$asset['version']
		);
	}
	...
	public static function render_shortcode( array $atts ): string {
		$atts = \shortcode_atts( [ 'title' => '' ], $atts, 'plume_chat' );
		$id   = 'plume-widget-' . \absint( \get_the_ID() );
		return '<div id="' . \esc_attr( $id ) . '" class="plume-widget" data-post-id="' . \absint( \get_the_ID() ) . '"></div>';
	}
}
